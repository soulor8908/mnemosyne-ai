...
export async function runAgent(opts: RunAgentOptions): Promise<string> {
  ...
  try {
    // 1. 加载近 7 日笔记（埋点：load-notes）
    const recentNotes = await traceStep(
      run.id,
      'load-notes',
      () => getRecentNotes(7),
      (result) => ({ noteCount: result?.length ?? 0 })
    );
    ...
    // 2. 计算语义关联（埋点：compute-similarity）
    const similarityStart = now();
    ...
    for (const re of recentEmbeddings) {
      ...
      for (const oe of allEmbeddings) {
        ...
        if (sim >= 0.6) {
          // 检查是否已有双链（埋点：check-bilink，best-effort 不阻断）
    ...
    await recordTrace(run.id, 'compute-similarity', 'success', similarityStart, {
      embeddingsCount: allEmbeddings.length,
      proposalsFound: linkProposals.length,
    });
    ...
    // 3. 直接创建高置信度链接提议（埋点：create-proposal）
    for (const p of topProposals) {
      try {
        ...
        proposalsCreated++;
        await recordTrace(run.id, 'create-proposal', 'success', proposalStart, {
          noteId: p.srcNote.id,
          confidence: p.confidence,
          proposalType: 'link',
        });
      } catch (err) {
        await recordTrace(run.id, 'create-proposal', 'failed', now(), {
          noteId: p.srcNote.id,
...
请以 JSON 数组格式返回：[{"front": "问题", "back": "答案"}]，不要其他文字。`;
        ...
        await recordTrace(run.id, 'llm-call', 'success', llmStart, {
          noteId: note.id,
          attempt: 1,
        });
        ...
        if (!parsed) {
          ...
          await recordTrace(run.id, 'retry-llm', 'success', retryStart, {
            noteId: note.id,
            attempt: 2,
          });

          const retryParseStart = now();
